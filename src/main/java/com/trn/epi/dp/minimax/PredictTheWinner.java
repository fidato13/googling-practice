...
public class PredictTheWinner {
    ...
    private static boolean predictTheWinner(int[] nums) {
        ...
        int sub = helper(0, nums.length - 1, nums);
        return (double) sub >= (double) total / 2;
    }
    ...
    private static int helper(int i, int j, int[] nums){
        ...
        int left = nums[i] + Math.min(helper(i + 2, j, nums), helper(i + 1, j - 1, nums));

        //if player chooses right
        int right = nums[j] + Math.min(helper(i, j - 2, nums), helper(i + 1, j - 1, nums));
